...
test("getParamLink - no matching token", (t) => {
  ...
  t.is(error.message, "No value found for parameter: name");
});
...
test("getNavClass", (t) => {
  ...
  t.is(getNavClass({ slug: "blog" }, { slug: "blog/post/1" }), "blog current");
});

test("nextItem - default case", (t) => {
  const list = [{ slug: "1" }, { slug: "2" }, { slug: "3" }] satisfies Page[];

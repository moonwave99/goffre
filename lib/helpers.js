...
export const getAsset = (asset, context) => {
    const { domain, env } = context.data.root;
    return env.mode === "prod" && domain
        ? `${domain}${asset.startsWith("/") ? "" : "/"}${asset}`
        : asset;
};
...
export const getLink = (page, context) => {
    ...
    const { config } = context.data.root;
    const uglyUrls = config.goffre?.uglyUrls;
